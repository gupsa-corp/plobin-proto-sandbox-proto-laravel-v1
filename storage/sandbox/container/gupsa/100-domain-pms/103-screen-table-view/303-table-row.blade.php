...
<td class="px-6 py-4 whitespace-nowrap" @click.stop data-column="name">
    <div class="flex items-center">
        <!-- Depth 들여쓰기 -->
        <?php if (isset($project['depth']) && $project['depth'] > 0): ?>
            <div style="width: <?= $project['depth'] * 20 ?>px;"></div>
        <?php endif; ?>
        
        <!-- 확장/축소 버튼 -->
        <?php if (isset($project['has_children']) && $project['has_children']): ?>
            <button class="mr-2 w-4 h-4 flex items-center justify-center text-gray-500 hover:text-gray-700" 
                    onclick="toggleProjectExpansion(<?= $project['id'] ?>, <?= $project['is_expanded'] ? 'false' : 'true' ?>)">
                <?php if ($project['is_expanded']): ?>
                    <!-- 축소 아이콘 (▼) -->
        ...
        <div class="flex-1">
            <!-- Depth 표시 (옵션) -->
            <?php if (isset($project['depth']) && $project['depth'] > 0): ?>
                <div class="text-xs text-gray-400 mb-1">레벨 <?= $project['depth'] ?></div>
            <?php endif; ?>
...
<td class="px-6 py-4 whitespace-nowrap text-sm font-medium" @click.stop>
    <div class="flex space-x-2">
        ...
        <?php if (!isset($project['depth']) || $project['depth'] < 9): ?>
            <button class="text-green-600 hover:text-green-900" 
                    onclick="openCreateChildModal(<?= $project['id'] ?>, '<?= addslashes($project['name']) ?>')">
                하위 추가
            </button>
        <?php endif; ?>
