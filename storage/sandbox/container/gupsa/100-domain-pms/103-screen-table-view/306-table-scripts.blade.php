...
// 프로젝트 확장/축소 토글
window.toggleProjectExpansion = async function(projectId, isExpanded) {
    try {
        const response = await fetch(`/api/sandbox/${getCurrentSandbox()}/api/projects/${projectId}/toggle`, {
            method: 'PATCH',
            headers: {
                'Content-Type': 'application/json',
            },
        ...
        if (result.success) {
            // 페이지 새로고침으로 트리 구조 다시 로드
            window.location.reload();
        } else {
            alert('확장/축소 변경 실패: ' + (result.message || '알 수 없는 오류'));
        }
    } catch (error) {
        console.error('확장/축소 에러:', error);
        alert('네트워크 오류가 발생했습니다: ' + error.message);
    }
};
...
window.openCreateChildModal = function(parentId, parentName) {
    document.getElementById('createProjectModal').classList.remove('hidden');
    // 부모 프로젝트 정보를 모달에 표시하거나 폼에 설정할 수 있음
    const modalTitle = document.querySelector('#createProjectModal h3');
    if (modalTitle) {
        modalTitle.textContent = `"${parentName}"의 하위 프로젝트 생성`;
    }
    
    // 부모 ID를 저장해두고 나중에 사용
    document.getElementById('createProjectForm').dataset.parentId = parentId;
};

// 하위 프로젝트 생성
window.createChildProject = async function(event, parentId) {
    event.preventDefault();
    ...
    try {
        const response = await fetch(`/api/sandbox/${getCurrentSandbox()}/api/projects/${parentId}/children`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
        ...
        if (result.success) {
            ...
            alert('하위 프로젝트 생성 실패: ' + (result.message || '알 수 없는 오류'));
        }
    } catch (error) {
        console.error('생성 에러:', error);
        alert('네트워크 오류가 발생했습니다: ' + error.message);
