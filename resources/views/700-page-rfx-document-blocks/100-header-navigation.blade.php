...
<div class="bg-white border-b">
    <div class="px-6 py-4">
        <nav class="flex space-x-8" aria-label="RFX Navigation">
            ...
            <a href="/rfx/requests" class="flex items-center px-3 py-2 text-sm font-medium rounded-md transition-colors text-gray-500 hover:text-gray-700 hover:bg-gray-50">
                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <a href="/rfx/forms" class="flex items-center px-3 py-2 text-sm font-medium rounded-md transition-colors text-gray-500 hover:text-gray-700 hover:bg-gray-50">
                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path>
                </svg>
                폼 실행
    ...
    <div class="max-w-[1920px] mx-auto px-4 py-4">
        <div class="flex items-center justify-between">
            ...
            <div>
                ...
                <p class="text-sm text-gray-500 mt-1">블록 뷰 - 페이지 {{ $pageNumber }}</p>
            </div>
        ...
        <!-- 페이지 네비게이션 -->
        @if($pagination && isset($pagination['total_pages']) && $pagination['total_pages'] > 1)
        <div class="mt-4 flex items-center justify-center space-x-2">
            <!-- 이전 페이지 -->
            @if($pageNumber > 1)
                <button wire:click="changePage({{ $pageNumber - 1 }})"
            ...
            @for($i = 1; $i <= $pagination['total_pages']; $i++)
                <button wire:click="changePage({{ $i }})"
                        class="px-3 py-1 rounded {{ $i === $pageNumber ? 'bg-blue-600 text-white' : 'bg-gray-200 hover:bg-gray-300' }}">
                    {{ $i }}
                </button>
            @endfor
